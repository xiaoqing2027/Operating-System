    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
       ...
       if(p->arrIndex < 3 ){
        ...
        // It should have changed its p->state before coming back.
          ...
          flag =1;
          average_burst = (p->bursts[p->arrIndex]+p->bursts[p->arrIndex-1]+p->bursts[p->arrIndex-2])/3;
          if(min_burst_time > average_burst ){
             min = p;
             min_burst_time = average_burst;
          }
...
thread_join(char **stack)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      //   cprintf("afterjionnnnnnnn%d\n", *stack);
      //   *stack = p->thread_top_stack;
      //   cprintf("afterjionnnnnnnn%d\n", *stack);
      //   cprintf("state%d\n", p->state);
      //   if(p->state == ZOMBIE ){
      //     int c =1;
